...
public class TagRepositoryWithBagRelationshipsImpl implements TagRepositoryWithBagRelationships {
    ...
    public List<Tag> fetchBagRelationships(List<Tag> tags) {
        return Optional.of(tags).map(this::fetchIds).orElse(Collections.emptyList());
    }

    Tag fetchIds(Tag result) {
        return entityManager
            .createQuery("select tag from Tag tag left join fetch tag.ids where tag.id = :id", Tag.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Tag> fetchIds(List<Tag> tags) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, tags.size()).forEach(index -> order.put(tags.get(index).getId(), index));
        List<Tag> result = entityManager
            .createQuery("select tag from Tag tag left join fetch tag.ids where tag in :tags", Tag.class)
